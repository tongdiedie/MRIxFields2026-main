...
def _fmt_time(seconds: float) -> str:
    """Format seconds as H:MM:SS for readable ETA display."""
    if seconds is None:
        return "unknown"
...
def _eta_from_progress(done_steps: int, total_steps: int, start_time: float, warmup_steps: int = 10) -> dict:
    """Estimate elapsed time and remaining time from global training progress.

    This function is intentionally lightweight: it only uses wall-clock time and
    integer step counts, so it has negligible overhead compared with GPU training.
    """
    ...
    if done_steps < warmup_steps:
        return {
            "elapsed": _fmt_time(elapsed),
            "eta_total": "warming_up",
            "steps_per_sec": done_steps / max(elapsed, 1e-9),
        }
    ...
    return {
        "elapsed": _fmt_time(elapsed),
        "eta_total": _fmt_time(eta),
        "steps_per_sec": steps_per_sec,
    }


def _effective_iters_per_epoch(loader_len: int, max_iters: int) -> int:
    """Return the number of optimization steps that will run per epoch."""
    return min(loader_len, max_iters) if max_iters else loader_len
...
def train_cut(cfg: dict, device: torch.device, use_dist: bool = False, resume_ckpt: str = None):
    ...
    iters_per_epoch = _effective_iters_per_epoch(loader_len, max_iters)
    total_steps = max(total_epochs - start_epoch, 0) * iters_per_epoch
    global_step = 0
    ...
    if is_main_process():
        ...
        print(f"Estimated total work for this run: {total_steps} steps")
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for i, (batch_a, batch_b) in enumerate(pbar):
            ...
            if is_main_process():
                global_step += 1
                eta_info = _eta_from_progress(global_step, total_steps, train_start_time)
                pbar.set_postfix(
                    G=f"{losses['loss_G']:.4f}",
                    D=f"{losses['loss_D']:.4f}",
                    done=f"{global_step}/{total_steps}",
                    elapsed=eta_info["elapsed"],
                    eta_total=eta_info["eta_total"],
                )
...
def train_cut_finetune(cfg: dict, device: torch.device, use_dist: bool = False, resume_ckpt: str = None):
    ...
    loader_len = len(loader)
    iters_per_epoch = _effective_iters_per_epoch(loader_len, max_iters)
    total_steps = max(epochs - start_epoch, 0) * iters_per_epoch
    global_step = 0
    train_start_time = time.time()
    if is_main_process():
        print(f"CUT fine-tuning: {data_cfg['source_field']} -> {data_cfg['target_field']}, "
              f"epochs {start_epoch+1}-{epochs}, {loader_len} iters/epoch"
              + (f" (capped at {max_iters})" if max_iters else "")
              + (f" (DDP: {world_size} GPUs)" if use_dist else ""))
        print(f"Estimated total work for this run: {total_steps} steps")
...
def train_stargan(cfg: dict, device: torch.device, use_dist: bool = False,
    ...
    train_start_time = time.time()
    start_step = step
    total_remaining_steps = max(total_iters - start_step, 0)
    ...
    if is_main_process():
        ...
        print(f"Estimated total work for this run: {total_remaining_steps} steps")
    epoch_count = 0
    while step < total_iters:
        ...
        for batch in loader:
            ...
            if is_main_process() and step % train_cfg.get("print_every", 1000) == 0:
                ...
                eta_info = _eta_from_progress(done_steps, total_remaining_steps, train_start_time)
                print(f"[{step}/{total_iters}] D: {d_loss.item():.4f}, "
                      f"G: {g_loss.item():.4f}, ds: {current_lambda_ds:.3f}, lr: {current_lr:.2e}, "
                      f"done: {done_steps}/{total_remaining_steps}, "
                      f"elapsed: {eta_info['elapsed']}, eta_total: {eta_info['eta_total']}")
...
def train_stargan_finetune(cfg: dict, device: torch.device, use_dist: bool = False,
    ...
    loader_len = len(loader)
    total_steps = max(epochs - start_epoch, 0) * loader_len
    global_step = 0
    train_start_time = time.time()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        epoch_loss = 0.0
        pbar = tqdm(loader, desc=f"Epoch {epoch+1}/{epochs}", total=loader_len) if is_main_process() else loader
